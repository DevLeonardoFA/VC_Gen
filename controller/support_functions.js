    ...
    for (let position = 0; position < elements.length; position++) {
        ...
        switch (type) {
            case 'param_group_open':
                returnElements += obj_param_group.gen_settings(type, name, base, admin, "open");
                break;

            case 'param_group_close':
                returnElements += obj_param_group.gen_settings(type, name, base, admin, "close");
                break;

            case 'textarea_html':
                returnElements += obj_textarea_html.gen_settings(type, name, base, admin);
                break;

            case 'attach_images':
                returnElements += obj_attach_images.gen_settings(type, name, base, admin);
                break;

            case 'dropdown':
                returnElements += obj_dropdown.gen_settings(type, name, base, admin, options);
                break;

            case 'checkbox':
                returnElements += obj_checkbox.gen_settings(type, name, base, admin, options);
                break;

            default:
                returnElements += obj_default_field.gen_settings(type, name, base, admin);
                break;
        }
    }
...
export function Populate_Variables(ID){
    ...
    for (let i = 0; i < elements_length.length; i++) {
        ...
        let field_options = [];
        let admin_label =   $(elements_length[i]).find('#admin_label').is(":checked");
        ...
        elements.push({type: type, field_name: field_name, field_slug: field_slug, field_options: field_options, image: image, admin_label : admin_label});

    }

    ID++;

    let settings = Gen_Settings(name, base, image, elements, ID);
